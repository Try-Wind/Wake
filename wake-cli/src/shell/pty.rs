...
use crate::fc::server::WakeSessionServer;
...
impl WakePtyManager {
    ...
    pub fn start_session(&mut self, shell: Shell, quiet: bool) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if pid == 0 {
            ...
            let io_server = WakeSessionServer::new(&self.session_id, 100, 1000); 
            io_server.start()?;

            self.inject_wake_hooks(&shell)?;
    ...
    fn inject_wake_hooks(&mut self, shell: &Shell) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if bytes_written == -1 {
            return Err("Failed to inject wake hooks".into());
        }
    ...
    fn setup_child_process(&self, shell: Shell, quiet: bool) -> ! {
        unsafe {
            ...
            // Set WAKE_SESSION_ID environment variable
            let session_env = std::ffi::CString::new("WAKE_SESSION_ID").unwrap();
    ...
    fn handle_io_forwarding(&self, io_server: WakeSessionServer, child_pid: i32) -> Result<(), Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_pty_manager_creation() {
        let pty = WakePtyManager::new().unwrap();
    ...
    fn test_unique_session_ids() {
        let pty1 = WakePtyManager::new().unwrap();
        thread::sleep(Duration::from_millis(1)); // Ensure different timestamp
        let pty2 = WakePtyManager::new().unwrap();
    ...
    fn test_create_pty_pair() {
        let result = WakePtyManager::create_pty_pair();
        assert!(result.is_ok());
    ...
    fn test_pty_pair_communication() {
        let (master_fd, slave_fd) = WakePtyManager::create_pty_pair().unwrap();
