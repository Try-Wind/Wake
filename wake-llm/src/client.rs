...
impl LlmClient {
    ...
    pub fn first_from_env() -> Option<Self> {
        if let Ok(provider) = std::env::var("WAKE_PROVIDER") {
...
impl LlmClient {
    ...
    pub async fn default_model(&self) -> Result<String, LlmError> {
        if let Ok(model) = std::env::var("WAKE_MODEL") {
            Ok(model)
        } else {
...
impl LlmClient {
    pub async fn chat(
        ...
        let response = self.provider.chat(request).await?.extract_think_content();
